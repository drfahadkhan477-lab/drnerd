...
heading above them, pages of two columns read column by column however the
PDF wrote them, and scanned pages — pictures of text — read by text
recognition, on the device), and taught one section at a time:
...
your device, so its pages and figures can be shown. Scanned pages are read
by Tesseract, fetched from jsDelivr the first time a PDF needs it — pinned
and integrity-checked like the PDF reader, and kept for offline use after
that — and the library names every page it read that way, so you know which
text came from recognition rather than the PDF itself.
