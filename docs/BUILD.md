...
node scripts/verify.js --pwa                           # → 1013 + 75 checks
```
...
Fifty-four patch scripts run in order against the export. Each applies a list of
...
Thirty-one suites, 1013 checks, plus 75 more on the split build. They run one at a
