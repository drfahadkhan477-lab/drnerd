...
The chain is 85 patch scripts, run in order against the export. Each applies a list of
...
| 69 | `figloadfade` | a figure fades in once its full-resolution image finishes loading, instead of popping in mid-decode — the hidden state is opt-in, added only on a path that has already committed to removing it on both `load` and `error`, so a failed load is never silently blank. **Not yet build-verified**: written and syntax-checked without the licensed export |
